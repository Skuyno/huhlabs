...
public class Main {
    public static void main(String[] args) {
        ...
        FunctionND interior = PenaltyFunctions.interiorPenalty(baseFunction, List.of(inequality), 1.0);
        FunctionND exterior = PenaltyFunctions.exteriorPenalty(baseFunction,
                List.of(inequality), List.of(equality), 1.0);

        Vector2d start = new Vector2d(-0.5, -0.5);
        double eps = 1e-6;
        int maxIter = 100;

        MDResult gdInterior = GradientDescent.findExtrema(interior, start, eps, 1.0, maxIter);
        MDResult cgInterior = ConjugateGradient.findExtrema(interior, start, eps, 1.0, maxIter);
